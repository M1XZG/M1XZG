...
updateprofile () {
    ...
    ./scripts/update-myhours-workflow-v2-afk.py $STEAM_GAME_ID
    if [ $? -ne 0 ]; then
...
#	fi
	...
	gh pr create --title "Update AFK Account VRC Hours - $TSTAMP" --body "Update of AFK Account VRChat hours via cron"
	sleep 5s
	gh pr merge --auto -m
	git checkout main
